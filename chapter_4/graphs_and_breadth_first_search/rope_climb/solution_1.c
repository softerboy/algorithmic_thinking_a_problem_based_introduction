...
typedef int rope[SIZE * 2];
typedef int positions[SIZE * 2];

void find_distances(int target_height, int jump_distance, int itching[], rope min_moves);
...
void find_distances(int target_height, int jump_distance, int itching[], rope min_moves) {
  ...
  int num_curr_positions, num_new_positions;
  int i, j, from_height;
  ...
  min_moves[0] = 0;
  cur_positions[0] = 0;
  num_curr_positions = 1;

  while (num_curr_positions > 0) {
    num_new_positions = 0;

    for (i = 0; i < num_curr_positions; ++i) {
    ...
    num_curr_positions = num_new_positions;
    for (i = 0; i < num_curr_positions; ++i)
