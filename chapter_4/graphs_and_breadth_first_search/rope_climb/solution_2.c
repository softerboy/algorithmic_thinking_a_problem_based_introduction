...
typedef int ropes[SIZE * 2][2];
typedef position positions[SIZE * 4];

void add_position_up(int from_height, int to_height, int max_height,
                     positions pos, int *num_pos, const int itching[],
                     ropes min_moves);

void add_position_down(int from_height, int to_height,
                       positions pos, int *num_pos, ropes min_moves);

void add_position_01(int from_height, positions pos, int *num_pos, ropes min_moves);

void add_position_10(int from_height, positions pos, int *num_pos, const int itching[], ropes min_moves);

void find_distances(int target_height, int jump_distance, int itching[], ropes min_moves);

void solve(int target_height, ropes min_moves);

int main() {
  int target_height, jump_distance, num_itching_sections;
  static int itching[SIZE * 2] = {0};
  static ropes min_moves;
...
void add_position_up(int from_height, int to_height, int max_height,
                     positions pos, int *num_pos, const int itching[], ropes min_moves) {
...
void add_position_10(int from_height, positions pos, int *num_pos,
                     const int itching[], ropes min_moves) {
  int distance = min_moves[from_height][1];
